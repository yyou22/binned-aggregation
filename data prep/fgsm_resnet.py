...
parser = argparse.ArgumentParser(description='FGSM Attack on CIFAR-10 with ResNet Models')
...
def attack(model, X_data, Y_data):
	...
	tensor_ = T.ToTensor()
	normalize = T.Normalize(mean, std) #nr
	...
	inv_normalize = T.Normalize(inv_mean, inv_std) #nr
	...
	for idx in range(start_idx, len(Y_data)):
		...
		perturbed_data = normalize(perturbed_data) #nr
		...
		#undo transformation
		perturbed_data = inv_normalize(perturbed_data) #nr
